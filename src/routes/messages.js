...
router.post('/', async function postMessages(req, res) {
	...
	for (var message of req.body) {
		...
		if (messageSent) {
			logger.log('debug', `${req.boxid}: ${req.method} ${req.originalUrl}: Already Sent Message: ${messageId}`);
		}
		else if (message.recipient.validated) {
			...
			logger.log('error', `${req.boxid}: ${req.method} ${req.originalUrl}: Failed on ${message.id}: No valid teacher: ${JSON.stringify(message.recipient)}`);	
		}
	}
	res.sendStatus(200);
});
